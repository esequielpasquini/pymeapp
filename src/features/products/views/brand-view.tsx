...
export async function BrandProductsView({
  brand,
  q,
  page,
  basePath,
  ...
  return (
    <div className="mx-auto max-w-2xl space-y-4 md:max-w-4xl lg:max-w-5xl">
      ...
      <div className="flex items-center gap-3">
        ...
        <div>
          <h1 className="text-xl font-semibold md:text-2xl">{brand}</h1>
          <p className="text-sm text-muted-foreground md:text-base">{total} productos</p>
        </div>
      </div>

      <SearchBox placeholder={`Buscar en ${brand}...`} />
      ...
      {q && products.length === 0 && (
        <div className="rounded-lg border border-dashed border-border p-6 text-center md:p-8">
          <p className="mb-3 text-sm text-muted-foreground md:text-base">
            No encontramos &quot;{q}&quot; en {brand}. Faltaba en el sistema?
          </p>
      ...
      <div className="grid grid-cols-1 gap-3 md:grid-cols-2">
        ...
        {!q && products.length === 0 && (
          <p className="py-8 text-center text-sm text-muted-foreground md:col-span-2">
            Esta marca todavia no tiene productos cargados.
          </p>
        )}
      </div>
      ...
      {totalPages > 1 && (
        <div className="flex flex-wrap items-center justify-center gap-2">
          {Array.from({ length: totalPages }, (_, i) => i + 1).map((p) => (
            ...
            >
              <Link
                href={`${basePath}/brand/${encodeURIComponent(brand)}?${
