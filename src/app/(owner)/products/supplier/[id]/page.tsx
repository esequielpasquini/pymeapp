...
export default async function ProductsBySupplierDetailPage({
  ...
  return (
    <div className="space-y-6">
      ...
      <ProductTable products={products} showCategory emptyMessage="Este proveedor todavia no tiene productos." />

      {totalPages > 1 && (
        <div className="flex items-center justify-center gap-2">
          {Array.from({ length: totalPages }, (_, i) => i + 1).map((p) => (
            <Button key={p} asChild variant={p === currentPage ? "default" : "outline"} size="sm">
              <Link href={`/products/supplier/${id}?${q ? `q=${encodeURIComponent(q)}&` : ""}page=${p}`}>
                {p}
              </Link>
            </Button>
          ))}
